...
import com.core.configuration.helper.ReactiveTokenHelper;
import com.core.configuration.interceptor.ClientBearerInterceptor;
import com.core.exception.GrpcErrorMapper;
...
public class GrpcHelper {
  ...
  public <R, V> Mono<ResponseEntity<Envelope<V>>> callGrpc(
    ...
    return Mono.fromCallable(supplier::get)
        ...
        .onErrorResume(StatusRuntimeException.class,
            ex ->
  ...
  public <StubT, RespT, ViewT> Mono<ResponseEntity<Envelope<ViewT>>> callGrpc(
    ...
    return ReactiveTokenHelper.currentBearerToken()
        ...
        .flatMap(token -> Mono.fromCallable(() -> {
                  Channel authed = ClientBearerInterceptor.withToken(channel,
                      token);
                ...
                .onErrorResume(StatusRuntimeException.class,
                    ex ->
                        Mono.just(com.core.exception.GrpcErrorMapper.<ViewT>toEnvelope(ex))
  ...
  public <StubT> Mono<ResponseEntity<Envelope<Void>>> callGrpcVoid(
    ...
    return ReactiveTokenHelper.currentBearerToken()
        ...
        .flatMap(token -> Mono.fromRunnable(() -> {
                ...
                .onErrorResume(StatusRuntimeException.class,
                    ex ->
                        Mono.just(com.core.exception.GrpcErrorMapper.<Void>toEnvelope(ex))
                            .map(env -> ResponseEntity.status(env.getStatus()).body(env)))
                .subscribeOn(Schedulers.boundedElastic())
        );
  }
