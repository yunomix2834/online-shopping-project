...
public class AddressServiceImpl implements IAddressService {
  ...
  public void update(
    ...
    mustOwn(a);
    addressMapper.patchAddressFromAddressUpdateRequestDto(a,
        request);
    addressRepository.save(a);
  }
  ...
  public Envelope.Page<AddressResponseDto> listByUser(
    ...
    List<AddressResponseDto> docs = addressRepository.findByUserId(userId)
        ...
        .toList();
    int total = docs.size();
    int from = Math.min(page * size,
        total);
    int to = Math.min(from + size,
        total);
    List<AddressResponseDto> slice = docs.subList(from,
        to);
